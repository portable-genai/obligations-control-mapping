...
The catalog id (``Rgc7``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
_OLD_PACKAGE = "obligations_control_mapping"
_OLD_ENV_PREFIX = "OBLIGATIONS_"
_OLD_BARE_PREFIX = "OBLIGATIONS"
_OLD_RESOURCE = "rgc7-svc"
_OLD_DIST = "obligations-control-mapping"
_OLD_CATALOG_ID = "Rgc7"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (OBLIGATIONS_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-obligations",
    )
