...
One env var (``OBLIGATIONS_PROFILE``) selects the adapter family for every
...
_PROFILE_ENV = "OBLIGATIONS_PROFILE"
_SETTINGS_ENV = "OBLIGATIONS_SETTINGS"
...
class ProfileChoice:
    ...
    The variable is ``OBLIGATIONS_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("OBLIGATIONS_PROFILE", "local")``: that fallback reads an
...
_PKG = "obligations_control_mapping"
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``OBLIGATIONS_SETTINGS``) that does not
...
REVIEW_ROUTING_ENV = "OBLIGATIONS_REVIEW_ROUTING"
