...
def _kit_citations(result: TriageResult) -> tuple[KitCitation, ...]:
    ...
    a graph node's own wording, so masking only the snippet let the identifier cross to the
...
def result_to_review(result: TriageResult, *, maker: str, tenant: str = "") -> Review:
    ...
    content. That bites hardest on the coverage path, whose subject IS the caller-supplied
    ``CoverageRequest.scope``: a subject should never carry personal data, but the redaction has
    to hold whether or not it does.
    ...
    return Review(
        action="obligations_control_mapping:triage",
        subject=subject,
        maker=maker,
        tenant=tenant,
        ...
        sod_group="obligations_control_mapping-maker-checker",
        case_ref=subject,
        # Producer-owned, tenant-scoped key so a retried delivery is idempotent at the console.
        source_key=f"obligations-control-mapping:{subject}:{result.severity.value}",
