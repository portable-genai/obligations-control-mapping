...
``OBLIGATIONS_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``OBLIGATIONS_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from ..domain.narration import NarrationService
from ..domain.obligations import (
    AssessmentService,
    CoverageAssessment,
    CrossTenantError,
...
_TOKEN_ENV = "OBLIGATIONS_S2S_TOKEN"
_INSECURE_DEMO_ENV = "OBLIGATIONS_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "OBLIGATIONS_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "OBLIGATIONS_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "OBLIGATIONS_S2S_AUDIENCE"
_API_HOST_ENV = "OBLIGATIONS_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``OBLIGATIONS_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``OBLIGATIONS_PROFILE`` must never be allowed to
    ...
    ``OBLIGATIONS_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "OBLIGATIONS_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "obligations-control-mapping"
...
_APP_TITLE = "Obligations to Control Mapping"
...
#
# Note what is NOT in this expression: OBLIGATIONS_S2S_TOKEN. A service
...
def _assessment_to_review(assessment: CoverageAssessment) -> TriageResult:
    """Bridge a coverage assessment onto the shared R8 review envelope.

    The review path is shared across the fleet and speaks the kernel result shape, so a
    consequential coverage assessment is projected onto it here, at the surface, rather than the
    pure domain importing a surface type. The fields carry across unchanged.
    """
    return TriageResult(
...
@app.post("/v1/coverage", response_model=CoverageResponse, tags=["artifacts"])
def coverage(
    ...
    unless it validates and every figure in it is one the engine produced. Rule R8: a result with
    any gap escalates and is routed here, in the same request, with the verified principal as the
    maker.

    The register is tenant-owned: the read is authorised against the VERIFIED principal's tenant,
    and a caller from another tenant is refused with 403 (never 404). The client-asserted actor and
    tenant in the request never enter this decision; ``principal`` came from the identity port.
    """
    try:
        authorize_register_access(principal.tenant)
    except CrossTenantError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc)) from exc

    graph = seed_graph()
    if request.accept_proposals:
        graph = accept_all_proposals(graph)
    if request.moved_sources:
        graph = apply_change_feed(graph, tuple(request.moved_sources))
