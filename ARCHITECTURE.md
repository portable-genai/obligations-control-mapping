# ARCHITECTURE: Obligations to Control Mapping (`obligations-control-mapping`)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`OBLIGATIONS_PROFILE`) swaps the
...
## Layout (`src/obligations_control_mapping/`)
